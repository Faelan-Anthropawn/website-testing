import { litematicToWorldEdit, createBlockStreamFromLitematic } from './litematic.js';
    ...
    blockStream
      ...
      .onBlock(block => {
        ...
        if (!paletteMap.has(block.blockState)) {
          paletteMap.set(block.blockState, paletteIndex++);
        }
        
        const idx = block.x + block.z * metadata.width + block.y * metadata.width * metadata.length;
        denseBlocks[idx] = paletteMap.get(block.blockState);
      })
