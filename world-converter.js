...
async function convertWorld() {
  ...
  try {
    ...
    showStatus('Creating block stream from world region...');
    convertButton.textContent = 'Streaming world data...';
    console.log('Creating block stream from world region...');
    
    const blockStream = await createBlockStreamFromWorld(
      mcaFiles, x1, y1, z1, x2, y2, z2, progressCallback
    );
    
    showStatus('Loading schematic from stream...');
    convertButton.textContent = 'Processing blocks...';
    console.log('Loading schematic from stream...');
    
    const schem = await loadSchematicFromStream(blockStream);
    
    console.log(`World region loaded via streaming: ${schem.width}x${schem.height}x${schem.length}`);
