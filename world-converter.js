...
import { VoxelRenderer } from './js/voxel-renderer.js';
...
const mirrorButtons = document.querySelectorAll('.toggle-btn');
...
async function handleFileSelect(file) {
  ...
  try {
    ...
    alert(`World loaded successfully!\nFound ${regionCount} region file(s).\nYou can now use the 3D Region Selector or enter coordinates manually.`);
...
async function convertWorld() {
  ...
  const mirrorX = mirrorButtons[0].classList.contains('active');
  const mirrorY = mirrorButtons[1].classList.contains('active');
  const mirrorZ = mirrorButtons[2].classList.contains('active');
...
const viewerInfoText = document.getElementById('viewer-info-text');
...
openViewerBtn.addEventListener('click', () => {
  ...
  if (!voxelRenderer) {
    voxelRenderer = new VoxelRenderer(viewerContainer);
    voxelRenderer.loadWorldData(mcaFiles);
    
    const coords = {
      x1: parseInt(x1Input.value) || 0,
      y1: parseInt(y1Input.value) || 60,
      z1: parseInt(z1Input.value) || 0,
      x2: parseInt(x2Input.value) || 20,
      y2: parseInt(y2Input.value) || 80,
      z2: parseInt(z2Input.value) || 20
    };
    
    if (!isNaN(coords.x1) && !isNaN(coords.y1) && !isNaN(coords.z1) &&
        !isNaN(coords.x2) && !isNaN(coords.y2) && !isNaN(coords.z2)) {
      voxelRenderer.setBoundingBoxCoordinates(
...
applySelectionBtn.addEventListener('click', () => {
  if (voxelRenderer) {
    const coords = voxelRenderer.getBoundingBoxCoordinates();
    x1Input.value = coords.x1;
    y1Input.value = coords.y1;
    z1Input.value = coords.z1;
    x2Input.value = coords.x2;
    y2Input.value = coords.y2;
    z2Input.value = coords.z2;
    
    viewerModal.classList.remove('active');
    alert('Coordinates applied! You can now proceed with conversion.');
  }
});
...
function updateViewerInfo() {
  if (voxelRenderer) {
    ...
    const length = Math.abs(coords.z2 - coords.z1);
    viewerInfoText.textContent = `Selection: ${width}x${height}x${length} blocks | Use mouse to rotate, scroll to zoom, drag handles to adjust bounding box`;
  }
  
  setTimeout(updateViewerInfo, 100);
}
