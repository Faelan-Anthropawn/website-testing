...
function drawFrame(canvas, ctx, width, height, bandData, songTitle, creatorName, image, frameIndex, fps) {
    ...
    for (let i = 0; i < bandData.length; i++) {
        visualizerState.bands[i] =
            visualizerState.bands[i] * 0.8 + bandData[i] * 0.2;
    }
    ...
    for (let i = 0; i <= points; i++) {
        ...
        const bandPos = nx * (visualizerState.bands.length - 1);
        const b0 = Math.floor(bandPos);
        const b1 = Math.min(b0 + 1, visualizerState.bands.length - 1);
        const t = bandPos - b0;

        let energy =
            visualizerState.bands[b0] * (1 - t) +
            visualizerState.bands[b1] * t;

        energy = Math.tanh(energy * 1.4);

        const centerBias = 1 - Math.abs(nx - 0.5) * 2;
        const phase = time * 5 + bandPos * 0.85;
