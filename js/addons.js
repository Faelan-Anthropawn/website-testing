...
function renderAddons() {
  ...
  container.innerHTML = addons.map(addon => `
    <button 
      class="addon-card flex flex-col items-center justify-center gap-2 rounded-lg bg-secondary-dark p-4 h-24 text-center transition-colors duration-300 hover:bg-primary/20 hover:text-white border border-white/10 hover:border-primary cursor-pointer"
      data-addon-id="${addon.id}"
      aria-label="View ${addon.title}"
    >
      <span class="material-symbols-outlined text-primary text-3xl">${addon.icon}</span>
      <span class="text-sm font-semibold">${addon.title}</span>
    </button>
  `).join('');
...
function openAddonModal(addon) {
  ...
  description.textContent = addon.fullDescription;
