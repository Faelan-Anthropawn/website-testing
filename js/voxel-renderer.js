...
export class VoxelRenderer {
  constructor(container) {
    ...
    this.lastCameraChunk = { x: null, z: null };
  ...
  async loadWorldData(mcaFiles) {
    ...
    for (const [key, region] of Object.entries(mcaFiles)) {
      ...
      try {
        ...
        for (const [chunkKey, chunkData] of chunks.entries()) {
          ...
          const globalChunkKey = `${Math.floor(worldChunkX / 16)},${Math.floor(worldChunkZ / 16)}`;
          
          this.chunkBlockData.set(globalChunkKey, {
            chunkData,
            worldChunkX,
            worldChunkZ,
            blocks: new Map()
        ...
        console.log(`Parsed region ${key}: ${chunks.size} chunks loaded`);
      } catch (error) {
    ...
    console.log(`Loaded ${Object.keys(mcaFiles).length} region(s) with chunk-based rendering`);
  }
  
  updateVisibleChunks() {
    const cameraChunkX = Math.floor(this.camera.position.x / 16);
    const cameraChunkZ = Math.floor(this.camera.position.z / 16);
    
    this.frustumMatrix.multiplyMatrices(this.camera.projectionMatrix, this.camera.matrixWorldInverse);
    this.frustum.setFromProjectionMatrix(this.frustumMatrix);
    
    const chunkMoved = this.lastCameraChunk.x !== cameraChunkX || this.lastCameraChunk.z !== cameraChunkZ;
    this.lastCameraChunk = { x: cameraChunkX, z: cameraChunkZ };
    
    const chunksInRange = new Set();
    const chunksToLoad = [];
    
    for (let cx = cameraChunkX - this.renderDistance; cx <= cameraChunkX + this.renderDistance; cx++) {
      for (let cz = cameraChunkZ - this.renderDistance; cz <= cameraChunkZ + this.renderDistance; cz++) {
        const chunkKey = `${cx},${cz}`;
        const distance = Math.sqrt((cx - cameraChunkX) ** 2 + (cz - cameraChunkZ) ** 2);
        
        if (distance <= this.renderDistance && this.chunkBlockData.has(chunkKey)) {
          chunksInRange.add(chunkKey);
          
          const chunkInfo = this.chunkBlockData.get(chunkKey);
          const isVisible = this.isChunkInFrustum(chunkInfo);
          
          if (isVisible) {
            const desiredLOD = this.calculateLOD(distance);
            const processedState = this.processedChunks.get(chunkKey);
            
            if (!processedState || processedState.lod !== desiredLOD) {
              chunksToLoad.push({ chunkKey, distance });
            }
          } else {
            if (this.renderedChunks.has(chunkKey) || this.processedChunks.has(chunkKey)) {
    ...
    for (const chunkKey of this.processedChunks.keys()) {
      if (!chunksInRange.has(chunkKey)) {
        this.unloadChunk(chunkKey);
        this.renderedChunks.delete(chunkKey);
        this.processedChunks.delete(chunkKey);
      }
    ...
    if (chunksToLoad.length > 0) {
      ...
      chunksToLoad.forEach(({ chunkKey, distance }) => {
        ...
        const blocksRendered = this.renderChunk(chunkKey, lod);
        
        this.processedChunks.set(chunkKey, { lod, isEmpty: blocksRendered === 0 });
        if (blocksRendered > 0) {
  ...
  isChunkInFrustum(chunkInfo) {
    const { worldChunkX, worldChunkZ } = chunkInfo;
    const minY = Math.max(60, this.boundingBoxMin.y - 10);
    const maxY = Math.min(100, this.boundingBoxMax.y + 10);
    
    const chunkBounds = new THREE.Box3(
      new THREE.Vector3(worldChunkX, minY, worldChunkZ),
      new THREE.Vector3(worldChunkX + 16, maxY, worldChunkZ + 16)
    );
    
    return this.frustum.intersectsBox(chunkBounds);
  }
  ...
  renderChunk(chunkKey, lod = 2) {
    const chunkInfo = this.chunkBlockData.get(chunkKey);
    if (!chunkInfo) return 0;
    
    const { chunkData, worldChunkX, worldChunkZ } = chunkInfo;
    const geometry = new THREE.BoxGeometry(1, 1, 1);
    const sampleRate = lod;
      ...
      for (let x = 0; x < 16; x += sampleRate) {
        for (let z = 0; z < 16; z += sampleRate) {
          ...
          if (block && !block.includes('air') && !block.includes('cave_air')) {
            const worldX = worldChunkX + x;
            const worldZ = worldChunkZ + z;
            
            if (this.shouldRenderBlock(chunkKey, x, y, z)) {
              const material = this.getBlockMaterial(block);
              const mesh = new THREE.Mesh(geometry, material);
              mesh.position.set(worldX, y, worldZ);
              mesh.userData = { chunkKey, lod };
              
              const blockKey = `${worldX},${y},${worldZ}`;
              chunkInfo.blocks.set(blockKey, mesh);
  ...
  shouldRenderBlock(chunkKey, x, y, z) {
    const neighbors = [
      [x + 1, y, z, 1, 0], [x - 1, y, z, -1, 0],
      [x, y + 1, z, 0, 0], [x, y - 1, z, 0, 0],
      [x, y, z + 1, 0, 1], [x, y, z - 1, 0, -1]
    ];
    
    const chunkInfo = this.chunkBlockData.get(chunkKey);
    if (!chunkInfo) return true;
    const { chunkData } = chunkInfo;
    
    for (const [nx, ny, nz, dx, dz] of neighbors) {
      if (ny < 0 || ny >= 256) {
        return true;
      }
      ...
      if (nx >= 0 && nx < 16 && nz >= 0 && nz < 16) {
        neighborBlock = getBlockFromChunk(chunkData, nx, ny, nz);
      } else {
        const [chunkX, chunkZ] = chunkKey.split(',').map(Number);
        const neighborChunkX = chunkX + (nx < 0 ? -1 : nx >= 16 ? 1 : 0);
        const neighborChunkZ = chunkZ + (nz < 0 ? -1 : nz >= 16 ? 1 : 0);
        const neighborChunkKey = `${neighborChunkX},${neighborChunkZ}`;
        const neighborChunkInfo = this.chunkBlockData.get(neighborChunkKey);
        
        if (neighborChunkInfo) {
          const localX = ((nx % 16) + 16) % 16;
          const localZ = ((nz % 16) + 16) % 16;
          neighborBlock = getBlockFromChunk(neighborChunkInfo.chunkData, localX, ny, localZ);
        } else {
          return true;
        }
      }
      
      if (!neighborBlock || neighborBlock.includes('air') || neighborBlock.includes('cave_air')) {
        return true;
      }
    }
  ...
  unloadChunk(chunkKey) {
    ...
    for (const [blockKey, mesh] of chunkInfo.blocks) {
      this.scene.remove(mesh);
      mesh.geometry.dispose();
      mesh.material.dispose();
      
      const index = this.voxelMeshes.indexOf(mesh);
      if (index > -1) {
        this.voxelMeshes.splice(index, 1);
      }
    }
    
    chunkInfo.blocks.clear();
  }
  
  setRenderDistance(distance) {
    this.renderDistance = Math.max(1, Math.min(10, distance));
    this.lastCameraChunk = { x: null, z: null };
    this.updateVisibleChunks();
  }
  ...
  animate() {
    ...
    if (this.chunkBlockData && this.chunkBlockData.size > 0) {
      this.updateVisibleChunks();
    }
