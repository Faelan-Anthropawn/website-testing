...
const normalizeInfo = document.getElementById('volume-normalize-info');
...
function updateNormalizeInfo() {
    ...
    const currentLUFS = rmsToLUFS(rms);
    const gainNeeded = TARGET_LUFS - currentLUFS;
    
    normalizeInfo.textContent = `Current: ${currentLUFS.toFixed(1)} LUFS (Target: ${TARGET_LUFS} LUFS)`;
}
...
function startPlayback(startTime = 0) {
    ...
    volumeSourceNode.buffer = volumeAudioBuffer;
    ...
    volumeGainNode.gain.value = gain;
...
if (volumeUpBtn) {
    volumeUpBtn.addEventListener('click', () => {
        ...
        volumeLevelInput.value = Math.min(100, current + 10);
    });
}
