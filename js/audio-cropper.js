...
const fadeToggle = document.getElementById('cropper-fade-toggle');
const fadeDurationContainer = document.getElementById('cropper-fade-duration-container');
...
playBtn.addEventListener('click', () => {
    if (isPlaying) {
        ...
        const fadeIn = fadeInEnabled ? parseFloat(fadeInDuration.value) || 0 : 0;
        const fadeOut = fadeEnabled ? parseFloat(fadeOutDuration.value) || 0 : 0;
...
fadeInToggle.addEventListener('click', () => {
    ...
    fadeInToggle.classList.toggle('active', fadeInEnabled);
...
fadeToggle.addEventListener('click', () => {
    ...
    fadeToggle.textContent = fadeEnabled ? 'ON' : 'OFF';
    updateFadeDurationVisibility();
});

function updateFadeDurationVisibility() {
    if (fadeInEnabled || fadeEnabled) {
        fadeDurationContainer.classList.remove('hidden');
    } else {
        fadeDurationContainer.classList.add('hidden');
    }
}
...
cropBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        const fadeInSecs = fadeInEnabled ? parseFloat(fadeInDuration.value) || 0 : 0;
        const fadeOutSecs = fadeEnabled ? parseFloat(fadeOutDuration.value) || 0 : 0;
